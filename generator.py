...
def generateDataPackage(output_from_parsed_template, location, config_data):
    ...
    #####
    # Convert yaml to json datapackage.json (metadata)
    ...
    with open("output/temp.datapackage.json", 'r') as tempfile, \
            open("output/%s/preview.geojson" % name, 'r') as previewfile, \
            open("output/%s/datapackage.json" % name, 'w') as dpfile:
        data = json.load(tempfile)
        feed = json.load(previewfile)
        data['views'][0]['spec']['bounds'] = bbox
        data['resources'][0]['data']['features'] = feed['features']
        if 'as_circle' in config_data:
            data['resources'][0]['mediatype'] = "application/" + \
                "vnd.simplestyle-extended"
        json.dump(data, dpfile)
